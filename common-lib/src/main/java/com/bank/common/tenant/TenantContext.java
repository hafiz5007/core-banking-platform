...
/**
 * Request-scoped holder for the current organization (tenant). Populated by
 * {@link TenantContextFilter} from the {@code X-Organization-Id} header (mapped from the JWT
 * {@code org} claim at the gateway in production). Falls back to {@link #DEFAULT} when absent, so
 * single-tenant deployments work unchanged. See ADR-002.
 */
