 ...
 * around every listener callback. The MDC is a {@code ThreadLocal} and gRPC may run
 * {@code startCall} and the subsequent callbacks on different threads, so setting it once in
 * {@code interceptCall} would leak the value onto a pooled thread and lose it on the thread that
 * actually runs the handler. Each callback therefore sets and clears it itself.
 *
 * <p>A call arriving without an id gets a fresh one, exactly as {@link CorrelationIdFilter} does for
 * an untagged HTTP request.
 */
