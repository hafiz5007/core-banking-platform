...
class RateLimitFilterTest {
    ...
    void allowsRequestsUpToTheLimit() throws Exception {
        ...
        for (int i = 0; i < 3; i++) {
            filter.doFilter(request("/api/v1/accounts", "10.0.0.1"), new MockHttpServletResponse(), chain);
        }
    ...
    void rejectsWithTooManyRequestsOnceTheLimitIsExceeded() throws Exception {
        ...
        for (int i = 0; i < 2; i++) {
            filter.doFilter(request("/api/v1/accounts", "10.0.0.2"), new MockHttpServletResponse(), chain);
        }
    ...
    void actuatorProbesAreNeverRateLimited() throws Exception {
        ...
        for (int i = 0; i < 5; i++) {
            filter.doFilter(request("/actuator/health", "10.0.0.8"), new MockHttpServletResponse(), chain);
        }
