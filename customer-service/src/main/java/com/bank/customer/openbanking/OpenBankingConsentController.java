...
public class OpenBankingConsentController {
    ...
    @PostMapping
    public ResponseEntity<ConsentResponse> request(@Valid @RequestBody RequestConsentRequest request) {
        OpenBankingConsent consent = service.request(
                request.consentReference(), request.customerId(), request.tpp(),
                request.scopes(), request.validityDays() == 0 ? 90 : request.validityDays());
        return ResponseEntity.created(URI.create("/api/v1/open-banking/consents/" + consent.getConsentReference()))
    ...
    public ConsentResponse authorise(@PathVariable String reference, @Valid @RequestBody AuthoriseRequest request) {
    ...
    public record ConsentResponse(String consentReference, UUID customerId, String tpp, String scopes,
                                  ConsentStatus status, String scaReference, Instant expiresAt) {
        static ConsentResponse from(OpenBankingConsent c) {
            return new ConsentResponse(c.getConsentReference(), c.getCustomerId(), c.getTpp(),
                    c.getScopes(), c.getStatus(), c.getScaReference(), c.getExpiresAt());
        }
    }
}
