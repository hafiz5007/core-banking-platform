...
public class OpenBankingConsentService {
  ...
  @Transactional
  public OpenBankingConsent request(
    ...
    if (repository.existsByConsentReference(consentReference)) {
      throw new BusinessException(
          ErrorCode.DUPLICATE_REQUEST, "Consent reference already exists: " + consentReference);
    }
    Instant expiresAt = Instant.now().plus(validityDays, ChronoUnit.DAYS);
    OpenBankingConsent consent =
        OpenBankingConsent.request(consentReference, customerId, tpp, scopes, expiresAt);
    log.info(
        "Open Banking consent {} requested for customer {} by {}",
        consentReference,
        customerId,
        tpp);
  ...
  public OpenBankingConsent get(String consentReference) {
    OpenBankingConsent consent =
        repository
            .findByConsentReference(consentReference)
            .orElseThrow(
                () -> new ResourceNotFoundException("Consent not found: " + consentReference));
