...
public class CustomerController {
    ...
    public ResponseEntity<CustomerResponse> onboard(@Valid @RequestBody OnboardCustomerRequest request) {
        Customer customer = customerService.onboard(new OnboardCommand(
                request.firstName(), request.lastName(), request.dateOfBirth(),
                request.nationality(), request.email(), request.phone(), request.taxId(),
                request.dataProcessingConsent(), request.channel()));
    ...
    public List<ChangeLogResponse> changeLog(@PathVariable UUID id) {
        ...
        return changeLog.history("Customer", id.toString()).stream().map(ChangeLogResponse::from).toList();
    }
    ...
    public ResponseEntity<ConsentResponse> recordConsent(
        ...
        Consent consent = customerService.recordConsent(
