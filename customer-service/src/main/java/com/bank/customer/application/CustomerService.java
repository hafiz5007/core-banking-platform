 ...
 * sanctions/PEP screening through outbound ports, assign a risk rating, and resolve the final state.
 */
@Service
public class CustomerService {
    ...
    public Customer onboard(OnboardCommand cmd) {
        Customer customer = Customer.register(
                ...
                generateUniqueCif(),
                cmd.firstName(), cmd.lastName(), cmd.dateOfBirth(),
                cmd.nationality(), cmd.email(), cmd.phone(), cmd.taxId());

        // 1. KYC verification.
        KycVerificationResult kyc = kycVerificationPort.verify(
                new KycVerificationRequest(cmd.firstName(), cmd.lastName(), cmd.nationality(), cmd.taxId()));
        customer.recordKyc(kyc.status(), kyc.evidenceRef());

        // 2. Sanctions / PEP screening — block on a positive match.
        ScreeningResult screening = screeningPort.screen(
        ...
        RiskRating rating = RiskRatingPolicy.rate(cmd.nationality(), cmd.taxId() == null || cmd.taxId().isBlank());
        ...
        if (cmd.dataProcessingConsent()) {
            consentRepository.save(new Consent(
                    saved.getId(), ConsentType.DATA_PROCESSING, true, cmd.channel()));
        }

        changeLog.record("Customer", saved.getId().toString(),
                com.bank.customer.domain.ChangeType.CREATE, null,
                "Onboarded customer " + saved.getCif() + " status " + saved.getStatus());

        log.info("Onboarded customer cif={} status={} kyc={} screeningMatch={} risk={} org={}",
                saved.getCif(), saved.getStatus(), saved.getKycStatus(),
                saved.isScreeningMatch(), saved.getRiskRating(), saved.getOrganizationId());
        return saved;
    }
