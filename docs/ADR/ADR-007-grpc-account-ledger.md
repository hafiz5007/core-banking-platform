...
**Implemented** for the debit path: `account-service` → `ledger-service` `PostJournalEntry`.

| Piece | Where |
| --- | --- |
| Contract | `ledger-grpc-api` module — `src/main/proto/ledger_posting.proto`, stubs generated by `protobuf-maven-plugin`. Its own module so neither service depends on the other's code, only on the contract. |
| Server | `ledger-service` — `adapter/in/grpc/LedgerPostingGrpcService` delegating to the same `LedgerService.post` use case the REST controller calls; `config/GrpcServerConfig` runs the server on its own port as a `SmartLifecycle`. |
| Client | `account-service` — `application/port/LedgerPostingPort` with `adapter/out/ledger/GrpcLedgerPostingAdapter`; channel and stub in `config/GrpcClientConfig`. |
| Use case | `AccountService.debitAccount(...)` behind `POST /api/v1/accounts/{id}/debit`. |
| Deployment | `docker-compose.yml`: ledger publishes 9090, account is configured with `LEDGER_POSTING=grpc`. |

Notes on what was decided while building it:

- **Amounts cross the wire as decimal strings**, not `double` or a scaled int. Protobuf has no
  decimal type, and the platform rule is that money is never a float. `"0.07"` reaches the ledger as
  exactly `0.07`; there is a test for that.
- **A debit is a balanced pair**: DEBIT the customer account, CREDIT `ledger.settlement-account`.
  The customer account alone is not a journal entry.
- **The posting runs inside the account's transaction.** If the ledger rejects the entry, the
  balance change rolls back, so the two cannot disagree. The cost is that a ledger outage fails the
  debit — deliberate: a debit that never reaches the ledger is worse than one that is refused. The
  ledger is idempotent on `idempotency_key`, so the retry is safe.
- **Status mapping is explicit**, mirroring the REST `GlobalExceptionHandler`: `NOT_FOUND`,
  `ALREADY_EXISTS`, `INVALID_ARGUMENT`, `FAILED_PRECONDITION` become business failures (4xx);
  `UNAVAILABLE` / `DEADLINE_EXCEEDED` stay infrastructure faults (5xx) so they are retried and
  alerted on rather than shown to a customer.
- **Plaintext, not TLS.** In-cluster traffic is assumed to be secured by the mesh; see the
  service-to-service item in `docs/PENDING_TASKS.md`. Swap credentials into `GrpcClientConfig` and
  `GrpcServerConfig` if that assumption does not hold.

**Not implemented:** the balance-query RPC in the original design. Only the posting path exists.
Reads still go over REST.
