...
public class NotificationService {
  ...
  @Transactional
  public Notification sendAlert(
      String recipient, NotificationType type, NotificationChannel channel, String message) {
    ...
    changeLog.record(
        ...
        com.bank.notification.domain.ChangeType.CREATE,
        null,
        type + " via " + channel);
    return saved;
  }
  ...
  public UUID issueOtp(String recipient, NotificationChannel channel) {
    ...
    OtpChallenge challenge =
        new OtpChallenge(
            recipient, hash(code), Instant.now().plus(OTP_TTL_SECONDS, ChronoUnit.SECONDS));
  ...
  public boolean verifyOtp(UUID challengeId, String code) {
    OtpChallenge challenge =
        otpRepository
            ...
            .orElseThrow(
                () ->
                    new BusinessException(ErrorCode.RESOURCE_NOT_FOUND, "OTP challenge not found"));
