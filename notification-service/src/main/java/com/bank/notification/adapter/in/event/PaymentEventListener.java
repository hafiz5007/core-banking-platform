 ...
 *
 * <p>notification-service is the only service with an outbound notification concern; payment-service
 * publishes for domain reasons and knows nothing about alerts.
 *
 * <p>Kafka delivers at least once, so every record is checked against {@link ProcessedNotification} before
 * anything is sent. The check and the send share one transaction: either the alert is stored and the
 * record marked handled, or neither happens and redelivery retries cleanly.
 */
@Component
...
public class PaymentEventListener {
    ...
    public void onPaymentEvent(@Payload String payload,
        ...
        if (processedNotifications.existsByTopicAndPartitionIdAndRecordOffset(topic, partition, offset)) {
        ...
        try {
            ...
            log.error("Unreadable payment event at {}-{}@{}; skipping: {}", topic, partition, offset,
                    e.getMessage());
            processedNotifications.save(new ProcessedNotification(topic, partition, offset, "UNPARSEABLE"));
            return;
        }

        notificationService.sendAlert(
                event.debtor(), NotificationType.TRANSACTION_ALERT, NotificationChannel.EMAIL,
                message(event));
        processedNotifications.save(new ProcessedNotification(topic, partition, offset, event.status()));
    ...
    private static String message(PaymentEvent event) {
        return "Payment of %s %s to %s is %s."
                .formatted(event.amount().toPlainString(), event.currency(),
                        event.creditor(), event.status().toLowerCase());
    }
    ...
    private PaymentEvent parse(String payload) {
        try {
            ...
            return new PaymentEvent(
                    text(node, "paymentId"), text(node, "debtor"), text(node, "creditor"),
                    new BigDecimal(text(node, "amount")), text(node, "currency"), text(node, "status"));
        } catch (Exception e) {
    ...
    private record PaymentEvent(String paymentId, String debtor, String creditor,
