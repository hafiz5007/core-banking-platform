...
/**
 * The consumer half of the event backbone (ADR-014): a payment event on the topic becomes a
 * customer notification, exactly once even though Kafka delivers at least once.
 */
class PaymentEventListenerIT extends AbstractIntegrationTest {
  ...
  @Autowired PaymentEventListener listener;

  @Autowired NotificationRepository notifications;

  @Autowired ProcessedNotificationRepository processedNotifications;
  ...
  private long alertsFor(String recipient) {
    return notifications.findAll().stream().filter(n -> recipient.equals(n.getRecipient())).count();
  }
  ...
  void redeliveryOfTheSameRecordDoesNotAlertTwice() {
    ...
    assertThat(processedNotifications.existsByTopicAndPartitionIdAndRecordOffset(TOPIC, 0, 4242L))
        .isTrue();
  }
  ...
  void anUnreadableRecordIsRecordedAndSkippedRatherThanBlockingThePartition() {
    ...
    assertThat(processedNotifications.existsByTopicAndPartitionIdAndRecordOffset(TOPIC, 0, 6001L))
        .isTrue();
  }
}
