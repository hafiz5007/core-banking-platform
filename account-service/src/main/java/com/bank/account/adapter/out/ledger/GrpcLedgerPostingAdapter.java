...
public class GrpcLedgerPostingAdapter implements LedgerPostingPort {
    ...
    public GrpcLedgerPostingAdapter(LedgerPostingGrpc.LedgerPostingBlockingStub stub,
    ...
    public String postDebit(String accountCode, Money amount, String narrative, String idempotencyKey) {
        PostJournalEntryRequest request = PostJournalEntryRequest.newBuilder()
                ...
                .build();
        try {
            PostJournalEntryResponse response = stub
                    .withDeadlineAfter(deadlineMs, TimeUnit.MILLISECONDS)
                    .postJournalEntry(request);
            log.info("Posted debit of {} on {} to ledger entry {}", amount, accountCode, response.getEntryId());
    ...
    private static RuntimeException translate(StatusRuntimeException e, String accountCode) {
        String detail = e.getStatus().getDescription() == null ? e.getStatus().getCode().name()
                : e.getStatus().getDescription();
        return switch (e.getStatus().getCode()) {
            case NOT_FOUND -> new BusinessException(ErrorCode.BUSINESS_RULE_VIOLATION,
                    "Ledger account not found while posting " + accountCode + ": " + detail);
            case INVALID_ARGUMENT -> new BusinessException(ErrorCode.VALIDATION_FAILED,
                    "Ledger rejected the posting: " + detail);
            ...
            default -> new LedgerUnavailableException(
