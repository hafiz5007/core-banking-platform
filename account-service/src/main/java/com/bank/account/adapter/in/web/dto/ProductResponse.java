...
public record ProductResponse(
  ...
  public static ProductResponse from(Product p) {
    return new ProductResponse(
        ...
        p.getAccountType(),
        p.getCurrencyCode(),
        p.getInterestRatePercent().toPlainString(),
        p.getMonthlyFee().toPlainString(),
        p.getMinBalance().toPlainString(),
        p.getDailyLimit().toPlainString(),
        p.getOverdraftLimit().toPlainString(),
        p.getStatus());
  }
}
