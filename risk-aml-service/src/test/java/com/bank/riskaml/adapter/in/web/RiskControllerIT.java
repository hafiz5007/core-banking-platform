...
class RiskControllerIT extends AbstractIntegrationTest {
    ...
    private EvaluationResult evaluate(BigDecimal amount, String country) {
        return rest.postForEntity("/api/v1/risk/evaluate",
                new EvaluateRequest(ref("TXN"), ref("ACC"), amount, "USD", country),
                EvaluationResult.class).getBody();
    }
    ...
    void theOpenedCaseIsReadableAndCarriesTheRuleThatFired() {
        ...
        ResponseEntity<CaseResponse> found = rest.getForEntity("/api/v1/risk/cases/" + caseId, CaseResponse.class);
    ...
    void closingACaseRecordsTheResolutionAndSarFlag() {
        ...
        ResponseEntity<CaseResponse> closed = rest.postForEntity("/api/v1/risk/cases/" + caseId + "/close",
                new CloseCaseRequest("Reviewed: legitimate property purchase", true), CaseResponse.class);
    ...
    void aCaseCannotBeClosedTwice() {
        UUID caseId = evaluate(new BigDecimal("25000.00"), "GB").caseId();
        rest.postForEntity("/api/v1/risk/cases/" + caseId + "/close",
                new CloseCaseRequest("first", false), CaseResponse.class);

        ResponseEntity<String> second = rest.postForEntity("/api/v1/risk/cases/" + caseId + "/close",
                new CloseCaseRequest("second", false), String.class);
    ...
    void anOverLongTransactionReferenceIsRejectedAsABadRequest() {
        ResponseEntity<String> response = rest.postForEntity("/api/v1/risk/evaluate",
