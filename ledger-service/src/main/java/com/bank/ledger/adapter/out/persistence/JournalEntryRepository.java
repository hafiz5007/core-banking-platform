...
public interface JournalEntryRepository extends JpaRepository<JournalEntry, UUID> {
    ...
    /**
     * Load an entry with its lines in one query. Needed because { open-in-view} is off: the
     * lazy { lines} collection would otherwise be mapped to a response after the transaction
     * has closed. Also avoids the N+1 the plain finder would cause.
     */
