...
/**
 * gRPC inbound adapter for ledger posting (ADR-007). A second transport in front of the same {@link
 * LedgerService#post} use case the REST controller calls — no business logic lives here, only
...
public class LedgerPostingGrpcService extends LedgerPostingGrpc.LedgerPostingImplBase {
  ...
  public void postJournalEntry(
      PostJournalEntryRequest request, StreamObserver<PostJournalEntryResponse> responseObserver) {
    try {
  ...
  private static PostingCommand toCommand(PostJournalEntryRequest request) {
    ...
    if (request.getLinesCount() == 0) {
      throw Status.INVALID_ARGUMENT
          .withDescription("at least one line is required")
          .asRuntimeException();
    }
    List<LineCommand> lines =
        request.getLinesList().stream().map(LedgerPostingGrpcService::toLine).toList();
    return new PostingCommand(
        request.getIdempotencyKey(),
        request.getNarrative(),
        toValueDate(request.getValueDate()),
        lines);
  }
  ...
  private static LineCommand toLine(com.bank.ledger.grpc.v1.JournalLine line) {
    if (line.getAccountCode().isBlank()) {
      throw Status.INVALID_ARGUMENT
          .withDescription("account_code is required")
          .asRuntimeException();
    }
    Direction direction =
        switch (line.getDirection()) {
          ...
          default ->
              throw Status.INVALID_ARGUMENT
                  .withDescription("direction must be DEBIT or CREDIT")
                  .asRuntimeException();
        };
  ...
  private static BigDecimal toAmount(String raw) {
    try {
      ...
      throw Status.INVALID_ARGUMENT
          .withDescription("amount is not a decimal: " + raw)
          .asRuntimeException();
    }
  }
  ...
  private static LocalDate toValueDate(String raw) {
    ...
    try {
      ...
      throw Status.INVALID_ARGUMENT
          .withDescription("value_date must be ISO-8601 (yyyy-MM-dd): " + raw)
          .asRuntimeException();
    }
  }
}
