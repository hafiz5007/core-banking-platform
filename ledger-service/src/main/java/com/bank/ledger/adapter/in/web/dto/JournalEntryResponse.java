...
public record JournalEntryResponse(
    ...
    public record LineView(String accountCode, Direction direction, String amount, String currencyCode) {
    }

    public static JournalEntryResponse from(JournalEntry e) {
        List<LineView> lines = e.getLines().stream()
                .map(l -> new LineView(
                        l.getAccountCode(), l.getDirection(),
                        l.getAmount().toPlainString(), l.getCurrencyCode()))
                .toList();
        return new JournalEntryResponse(
                e.getId(), e.getIdempotencyKey(), e.getNarrative(), e.getValueDate(),
                e.getStatus(), e.getReversalOfId(), lines, e.getCreatedAt());
    }
}
