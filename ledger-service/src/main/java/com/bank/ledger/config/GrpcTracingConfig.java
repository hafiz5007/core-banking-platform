 ...
 * the outermost interceptor. Ordering this one last therefore makes it run first, so a call
 * rejected by the auth interceptor is still logged against the caller's correlation id rather than
 * anonymously.
 */
@Configuration
