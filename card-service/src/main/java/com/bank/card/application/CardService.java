...
/**
 * Card issuance, customer controls, and real-time authorization (FR-CRD-001/002/004).
 * Authorizations place a hold subject to usage controls; settlement converts the hold to a ledger
 * posting (debit the customer account, credit the card settlement account), reversal releases the
 * hold (FR-CRD-003).
 */
@Service
public class CardService {
  ...
  public Card issue(String accountCode, Money openingAvailable) {
    ...
    changeLog.record(
        ...
        com.bank.card.domain.ChangeType.CREATE,
        null,
        "Issued card ****" + lastFour + " for account " + accountCode);
  ...
  /**
   * Real-time authorization: approve and place a hold, or decline if
   * blocked/controlled/insufficient.
   */
  @Transactional
  public Authorization authorize(
      UUID cardId, Money amount, String merchant, CardChannel channel, boolean international) {
    ...
    try {
      ...
      Authorization auth =
          new Authorization(cardId, amount, merchant, AuthorizationStatus.APPROVED);
      log.info("Authorized {} on card {} via {} at {}", amount, cardId, channel, merchant);
      return authorizationRepository.save(auth);
    } catch (BusinessException e) {
      Authorization declined =
          new Authorization(cardId, amount, merchant, AuthorizationStatus.DECLINED);
  ...
  public Authorization settle(UUID authorizationId) {
    ...
    if (auth.getStatus() != AuthorizationStatus.APPROVED) {
      throw new BusinessException(
          ErrorCode.BUSINESS_RULE_VIOLATION, "Only an approved authorization can be settled");
    }
    ...
    UUID entryId =
        ledgerPort.postTransfer(
            new TransferCommand(
                "card-" + auth.getId(),
                "Card settlement " + auth.getMerchant(),
                card.getAccountCode(),
                settlementAccount,
                auth.money()));
    auth.settle();
  ...
  public Authorization reverse(UUID authorizationId) {
    ...
    if (auth.getStatus() != AuthorizationStatus.APPROVED) {
      throw new BusinessException(
          ErrorCode.BUSINESS_RULE_VIOLATION, "Only an approved authorization can be reversed");
    }
    Card card = getCard(auth.getCardId());
