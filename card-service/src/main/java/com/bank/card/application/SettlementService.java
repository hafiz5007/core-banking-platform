...
/**
 * Reconciles a clearing/settlement file from the card scheme against our authorizations (FR-CRD-003).
 * Each entry references an authorization; a matched, approved authorization is settled (posting to
 * the ledger), while anything that cannot be matched is reported as an exception for investigation.
 */
@Service
    ...
    public ReconciliationSummary reconcile(List<ClearingEntry> entries) {
        ...
        for (ClearingEntry entry : entries) {
            try {
                ...
                unmatched++;
                log.warn("Unmatched clearing entry {} (auth {}): {}",
                        entry.networkRef(), entry.authorizationId(), e.getMessage());
            }
        }
