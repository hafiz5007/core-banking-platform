...
public class Card {
    ...
    private Card(String cardToken, String lastFour, String accountCode, Money openingAvailable) {
        ...
        this.perTransactionLimit = BigDecimal.ZERO.setScale(openingAvailable.currency().getDefaultFractionDigits());
        this.createdAt = now;
        this.updatedAt = now;
    }

    public static Card issue(String cardToken, String lastFour, String accountCode, Money openingAvailable) {
    ...
    public void updateControls(boolean online, boolean contactless, boolean atm,
    ...
    /**
     * Place a hold for an authorization, enforcing status, usage controls and available funds.
     * Throws {@link BusinessException} on any breach (the caller maps that to a DECLINE).
     */
    public void placeHold(Money amount, CardChannel channel, boolean international) {
        if (status != CardStatus.ACTIVE) {
            throw new BusinessException(ErrorCode.BUSINESS_RULE_VIOLATION, "Card is not active: " + status);
        }
        if (!channelEnabled(channel)) {
            throw new BusinessException(ErrorCode.BUSINESS_RULE_VIOLATION, "Channel disabled: " + channel);
        }
        ...
        if (perTransactionLimit.signum() > 0 && amount.amount().compareTo(perTransactionLimit) > 0) {
            throw new BusinessException(ErrorCode.BUSINESS_RULE_VIOLATION, "Exceeds per-transaction limit");
        }
        if (available().amount().compareTo(amount.amount()) < 0) {
            throw new BusinessException(ErrorCode.BUSINESS_RULE_VIOLATION, "Insufficient available balance");
        }
