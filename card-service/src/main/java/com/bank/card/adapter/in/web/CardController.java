...
public class CardController {
  ...
  public ResponseEntity<CardResponse> issue(@Valid @RequestBody IssueCardRequest request) {
    ...
    Card card =
        cardService.issue(request.accountCode(), Money.of(request.openingAvailable(), currency));
  ...
  public CardResponse updateControls(
      @PathVariable UUID id, @Valid @RequestBody UpdateControlsRequest request) {
    Currency currency = Currency.getInstance(request.currencyCode());
    Card card =
        cardService.updateControls(
            id,
            request.online(),
            request.contactless(),
            request.atm(),
            request.international(),
            Money.of(request.perTransactionLimit(), currency));
