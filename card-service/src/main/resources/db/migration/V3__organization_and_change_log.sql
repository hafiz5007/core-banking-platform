-- ADR-002 (tenancy) + ADR-001 (change history) rollout for card-service.

ALTER TABLE card ADD COLUMN organization_id VARCHAR(60) NOT NULL DEFAULT 'DEFAULT';
CREATE INDEX idx_card_organization ON card (organization_id);
