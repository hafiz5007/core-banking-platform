...
class SettlementAndControlsIT extends AbstractIntegrationTest {
    ...
    private CardResponse issue() {
        return rest.postForEntity("/api/v1/cards",
                new IssueCardRequest("ACC-1", "USD", new BigDecimal("500.00")), CardResponse.class).getBody();
    }

    private AuthorizationResponse authorize(UUID cardId, String amount, CardChannel channel, boolean intl) {
        return rest.postForEntity("/api/v1/cards/" + cardId + "/authorizations",
    ...
    void internationalControlGatesAuthorization() {
        ...
        rest.postForEntity("/api/v1/cards/" + card.id() + "/controls",
    ...
    void reconcilesClearingFileAgainstAuthorizations() {
        ...
        List<ClearingEntry> file = List.of(
                new ClearingEntry(auth.id(), "NET-1"),
                new ClearingEntry(UUID.randomUUID(), "NET-2")); // no matching authorization

        ResponseEntity<ReconciliationSummary> summary = rest.postForEntity(
                "/api/v1/cards/settlement/reconcile", file, ReconciliationSummary.class);
