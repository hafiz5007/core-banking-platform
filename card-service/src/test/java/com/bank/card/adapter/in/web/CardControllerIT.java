...
class CardControllerIT extends AbstractIntegrationTest {
  ...
  private CardResponse issue(String available) {
    return rest.postForEntity(
            "/api/v1/cards",
            new IssueCardRequest("ACC-1", "USD", new BigDecimal(available)),
            CardResponse.class)
        .getBody();
  }
  ...
  void issuesAndAuthorizesWithHold() {
    ...
    ResponseEntity<AuthorizationResponse> auth =
        rest.postForEntity(
            "/api/v1/cards/" + card.id() + "/authorizations",
            new AuthorizeRequest(
                new BigDecimal("30.00"), "USD", "Coffee Shop", CardChannel.POS, false),
    ...
    CardResponse after =
        rest.getForEntity("/api/v1/cards/" + card.id(), CardResponse.class).getBody();
  ...
  void declinesWhenInsufficientFunds() {
    ...
    ResponseEntity<AuthorizationResponse> auth =
        rest.postForEntity(
            "/api/v1/cards/" + card.id() + "/authorizations",
            new AuthorizeRequest(
                new BigDecimal("50.00"), "USD", "Electronics", CardChannel.POS, false),
            AuthorizationResponse.class);
  ...
  void reversalReleasesHold() {
    ...
    AuthorizationResponse auth =
        rest.postForEntity(
                "/api/v1/cards/" + card.id() + "/authorizations",
                new AuthorizeRequest(
                    new BigDecimal("40.00"), "USD", "Hotel", CardChannel.POS, false),
                AuthorizationResponse.class)
            .getBody();

    rest.postForEntity(
        "/api/v1/cards/authorizations/" + auth.id() + "/reversal",
        null,
        AuthorizationResponse.class);
