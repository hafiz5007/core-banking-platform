 ...
 * rate, accumulating in {@code accruedInterest} at high precision; capitalization rounds the
 * accrued amount to the currency, adds it to the principal, and resets the accrual.
 */
@Entity
...
public class InterestPosition {
  ...
  public static InterestPosition open(
      String accountCode,
      Currency currency,
      BigDecimal annualRatePercent,
      Money principal,
      DayCountBasis dayCount,
      LocalDate openedOn) {
    return new InterestPosition(
        accountCode, currency, annualRatePercent, principal, dayCount, openedOn);
  }
