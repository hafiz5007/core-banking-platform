 ...
 * (FR-LED-006). Capitalized interest and fees are posted to the ledger through the {@link LedgerPort}.
 */
@Service
public class InterestService {
    ...
    public InterestService(InterestPositionRepository positionRepository,
    ...
    @Transactional
    public InterestPosition openPosition(String accountCode, Currency currency, BigDecimal annualRatePercent,
                                         Money principal, DayCountBasis dayCount) {
        if (positionRepository.existsByAccountCode(accountCode)) {
            throw new BusinessException(ErrorCode.DUPLICATE_REQUEST,
                    "Interest position already exists for " + accountCode);
        }
        InterestPosition position = InterestPosition.open(
        ...
        changeLog.record("InterestPosition", saved.getAccountCode(),
                com.bank.interestfee.domain.ChangeType.CREATE, null,
    ...
    public InterestPosition getPosition(String accountCode) {
        return positionRepository.findByAccountCode(accountCode)
                .orElseThrow(() -> new ResourceNotFoundException("Interest position not found: " + accountCode));
    }
    ...
    public InterestPosition capitalize(String accountCode) {
        ...
        UUID entryId = ledgerPort.postTransfer(new TransferCommand(
                ...
                interestExpenseAccount, accountCode, capitalized));
    ...
    public FeeCharge applyFee(String accountCode, FeeType feeType, Money amount) {
        ...
        UUID entryId = ledgerPort.postTransfer(new TransferCommand(
                "fee-" + UUID.randomUUID(), feeType + " fee " + accountCode,
                accountCode, feeIncomeAccount, amount));
    ...
    public EodSummary runEndOfDay(LocalDate businessDate) {
        ...
        log.info("EOD {}: accrued interest on {}/{} positions", businessDate, accrued, positions.size());
