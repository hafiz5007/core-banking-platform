...
class AdminControllerIT extends AbstractIntegrationTest {
  ...
  void submitsAndApprovesUnderFourEyes() {
    ...
    ResponseEntity<ApprovalResponse> submitted =
        rest.postForEntity(
    ...
    ResponseEntity<ApprovalResponse> approved =
        rest.postForEntity(
            "/api/v1/admin/approvals/" + id + "/approve",
            new DecisionRequest(unique("checker"), null),
            ApprovalResponse.class);
  ...
  void rejectsAnApprovalWithAReason() {
    ...
    UUID id =
        rest.postForEntity(
                "/api/v1/admin/approvals",
                new SubmitApprovalRequest("FEE_WAIVER", "{}", maker),
                ApprovalResponse.class)
            .getBody()
            .id();

    ResponseEntity<ApprovalResponse> rejected =
        rest.postForEntity(
            "/api/v1/admin/approvals/" + id + "/reject",
            new DecisionRequest(unique("checker"), "insufficient justification"),
            ApprovalResponse.class);
  ...
  void makerCannotBeTheirOwnChecker() {
    ...
    UUID id =
        rest.postForEntity(
                "/api/v1/admin/approvals",
                new SubmitApprovalRequest("LIMIT_INCREASE", "{}", maker),
                ApprovalResponse.class)
            .getBody()
            .id();
    ...
    ResponseEntity<String> response =
        rest.postForEntity(
            "/api/v1/admin/approvals/" + id + "/approve",
            new DecisionRequest(maker, null),
            String.class);
  ...
  void everyDecisionIsWrittenToTheAuditTrail() {
    ...
    rest.postForEntity(
        "/api/v1/admin/approvals/" + id + "/approve",
        new DecisionRequest(checker, null),
        ApprovalResponse.class);

    ResponseEntity<java.util.List<AuditResponse>> makerTrail =
        rest.exchange(
            "/api/v1/admin/audit?actor=" + maker,
            HttpMethod.GET,
            null,
            new ParameterizedTypeReference<java.util.List<AuditResponse>>() {});
    ResponseEntity<java.util.List<AuditResponse>> checkerTrail =
        rest.exchange(
            "/api/v1/admin/audit?actor=" + checker,
            HttpMethod.GET,
            null,
            new ParameterizedTypeReference<java.util.List<AuditResponse>>() {});

    assertThat(makerTrail.getBody())
        .extracting(AuditResponse::action)
        .contains("APPROVAL_SUBMITTED");
    assertThat(checkerTrail.getBody())
        .extracting(AuditResponse::action)
        .contains("APPROVAL_APPROVED");
  }
