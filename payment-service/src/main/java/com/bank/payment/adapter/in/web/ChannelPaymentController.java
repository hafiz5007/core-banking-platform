...
public class ChannelPaymentController {
  ...
  @PostMapping("/aliases")
  public ResponseEntity<AliasResponse> registerAlias(
      @Valid @RequestBody RegisterAliasRequest request) {
    PaymentAlias alias =
        channelPaymentService.registerAlias(
  ...
  @PostMapping("/billers")
  public ResponseEntity<BillerResponse> registerBiller(
      @Valid @RequestBody RegisterBillerRequest request) {
    Biller biller =
        channelPaymentService.registerBiller(
            request.billerCode(), request.name(), request.settlementAccount());
    return ResponseEntity.status(201)
        .body(
            new BillerResponse(
                biller.getBillerCode(), biller.getName(), biller.getSettlementAccount()));
  }
  ...
  public ResponseEntity<PaymentResponse> payToAlias(@Valid @RequestBody P2pRequest request) {
    ...
    var payment =
        channelPaymentService.payToAlias(
