 ...
 *
 * <p>A row is marked PUBLISHED only after the broker has acknowledged it. If a publish fails the
 * row stays PENDING and is retried on the next pass, so an event is never lost — at the cost of
 * possibly being delivered twice, which is why consumers must be idempotent (ADR-014).
 *
 * <p>Publishing happens one row at a time and a failure stops the batch: events for an aggregate
 * are ordered, and skipping past a failure would publish a later event before an earlier one.
 */
@Component
  ...
  public void flush() {
    ...
    for (OutboxEvent event : pending) {
      try {
        ...
        log.warn(
            "Outbox publish failed for event {} ({}); leaving it PENDING to retry: {}",
            event.getId(),
            event.getEventType(),
            e.getMessage());
        break;
