 ...
 * any external scheme submission are delegated to a {@link PaymentRailHandler} chosen by the
 * {@link RoutingEngine}.
 *
 * <p>Steps run as discrete persisted transitions, not one big transaction, because the ledger
 * posting is a cross-service effect that must be undone by an explicit compensating reversal — never
 * by a database rollback the ledger would not see.
 */
@Service
public class PaymentService {
    ...
    public Payment initiate(InitiatePaymentCommand cmd) {
        ...
        if (existing.isPresent()) {
            log.info("Idempotent replay of payment key={} -> {}", cmd.idempotencyKey(), existing.get().getId());
            return existing.get();
        }
        ...
        Payment payment = Payment.received(
                cmd.idempotencyKey(), cmd.type(), cmd.debtorAccount(),
                cmd.creditorAccount(), cmd.amount(), cmd.narrative(), cmd.targetCurrencyCode());
        payment = paymentRepository.save(payment);
        changeLog.record("Payment", payment.getId().toString(), ChangeType.CREATE, null,
        ...
        if (screening.blocked()) {
            ...
            throw new BusinessException(ErrorCode.BUSINESS_RULE_VIOLATION,
                    "Payment blocked by screening: " + screening.reason());
        }
        ...
        try {
            ...
            paymentRepository.save(payment);
            throw new BusinessException(ErrorCode.BUSINESS_RULE_VIOLATION,
                    "Payment fulfilment failed for " + payment.getId());
        }
        ...
        try {
            ...
            payment = paymentRepository.save(payment);
            log.info("Confirmed payment {} ({} {}) via {}", payment.getId(),
                    payment.getAmount(), payment.getCurrencyCode(), payment.getType());
        } catch (RuntimeException e) {
            UUID reversalId = ledgerPort.reverse(payment.getLedgerEntryId(), "rev-" + cmd.idempotencyKey());
    ...
    /** Return/recall a settled payment: post a compensating reversal and mark it RETURNED (FR-PAY-007). */
    public Payment returnPayment(UUID id, String reason) {
        ...
        UUID reversalId = ledgerPort.reverse(payment.getLedgerEntryId(), "ret-" + payment.getIdempotencyKey());
