...
/**
 * Selects the {@link PaymentRailHandler} that fulfils a given {@link PaymentType}. Handlers
 * register themselves via the rails they support, so adding a new rail (e.g. cross-border in a
 * later sprint) is a matter of adding a handler bean — no change here.
 */
@Component
  ...
  public PaymentRailHandler handlerFor(PaymentType type) {
    ...
    if (handler == null) {
      throw new BusinessException(
          ErrorCode.BUSINESS_RULE_VIOLATION, "Payment rail not available: " + type);
    }
    return handler;
  }
}
