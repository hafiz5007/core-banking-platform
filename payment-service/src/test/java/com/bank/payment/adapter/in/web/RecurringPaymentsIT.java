...
class RecurringPaymentsIT extends AbstractIntegrationTest {
    ...
    void standingOrderExecutesAndAdvances() {
        var create = new CreateStandingOrderRequest("1000", "2000", new BigDecimal("50.00"),
                "USD", Frequency.MONTHLY, LocalDate.now(), null, "Rent");
        StandingOrderResponse order =
                rest.postForEntity("/api/v1/standing-orders", create, StandingOrderResponse.class).getBody();
        assertThat(order).isNotNull();
        LocalDate firstRun = order.nextRunDate();

        ResponseEntity<RunResponse> run = rest.postForEntity(
        ...
        StandingOrderResponse after =
                rest.getForEntity("/api/v1/standing-orders/" + order.id(), StandingOrderResponse.class).getBody();
    ...
    void directDebitCollectsWithinMandateAndRejectsOverLimit() {
        ...
        var createMandate = new CreateMandateRequest(ref, "1000", "2000", new BigDecimal("100.00"), "USD");
        ...
        ResponseEntity<PaymentResponse> ok = rest.postForEntity(
                "/api/v1/direct-debits/mandates/" + ref + "/collect",
                new CollectRequest(new BigDecimal("40.00"), "USD", "COLL-" + UUID.randomUUID(), "Utility"),
        ...
        ResponseEntity<String> over = rest.postForEntity(
                "/api/v1/direct-debits/mandates/" + ref + "/collect",
                new CollectRequest(new BigDecimal("150.00"), "USD", "COLL-" + UUID.randomUUID(), "Utility"),
                String.class);
