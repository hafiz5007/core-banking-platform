...
class ChannelPaymentIT extends AbstractIntegrationTest {
  ...
  void p2pResolvesAliasAndPays() {
    ...
    rest.postForEntity(
        "/api/v1/aliases", new RegisterAliasRequest(alias, AliasType.EMAIL, "2000"), Object.class);

    ResponseEntity<PaymentResponse> p2p =
        rest.postForEntity(
            "/api/v1/payments/p2p",
            new P2pRequest(
                "p2p-" + UUID.randomUUID(), "1000", alias, new BigDecimal("25.00"), "USD"),
  ...
  void billPaymentResolvesBillerAndPays() {
    ...
    rest.postForEntity(
        "/api/v1/billers", new RegisterBillerRequest(code, "City Power", "9000"), Object.class);

    ResponseEntity<PaymentResponse> bill =
        rest.postForEntity(
