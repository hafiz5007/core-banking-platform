...
class PaymentControllerIT extends AbstractIntegrationTest {
    ...
    private InitiatePaymentRequest request(String key, PaymentType type, String debtor, String creditor) {
        return new InitiatePaymentRequest(key, type, debtor, creditor,
                new BigDecimal("100.00"), "USD", "Test transfer", null);
    }

    @Test
    void initiatesAndConfirmsIntrabankPayment() {
        ResponseEntity<PaymentResponse> created = rest.postForEntity("/api/v1/payments",
                request("p-" + UUID.randomUUID(), PaymentType.INTRABANK, "1000", "2000"), PaymentResponse.class);
    ...
    void settlesDomesticRtgsViaScheme() {
        ResponseEntity<PaymentResponse> created = rest.postForEntity("/api/v1/payments",
                request("rtgs-" + UUID.randomUUID(), PaymentType.RTGS, "1000", "GB29NWBK60161331926819"),
    ...
    void replayWithSameKeyReturnsSamePayment() {
        ...
        PaymentResponse first = rest.postForEntity("/api/v1/payments",
                request(key, PaymentType.INTRABANK, "1000", "2000"), PaymentResponse.class).getBody();
        PaymentResponse second = rest.postForEntity("/api/v1/payments",
                request(key, PaymentType.INTRABANK, "1000", "2000"), PaymentResponse.class).getBody();
    ...
    void returnsASettledPayment() {
        PaymentResponse confirmed = rest.postForEntity("/api/v1/payments",
                request("ret-" + UUID.randomUUID(), PaymentType.INTRABANK, "1000", "2000"),
                PaymentResponse.class).getBody();
        assertThat(confirmed).isNotNull();

        ResponseEntity<PaymentResponse> returned = rest.postForEntity(
    ...
    void settlesCrossBorderWithFxViaSwift() {
        var crossBorder = new InitiatePaymentRequest("xb-" + UUID.randomUUID(), PaymentType.CROSS_BORDER,
                "1000", "DE89370400440532013000", new BigDecimal("100.00"), "USD", "Overseas invoice", "EUR");
    ...
    void blankDebtorIsRejected() {
        var bad = new InitiatePaymentRequest("bad-" + UUID.randomUUID(), PaymentType.INTRABANK,
                "", "2000", new BigDecimal("100.00"), "USD", "x", null);
